...
class OrphanageDonationCubit extends Cubit<OrphanageDonationStateState> {
  ...
  OrphanageDonationCubit(this._orphanageDonationRemoteDataSource)
    : super(OrphanageDonationStateInitial());

  void getRequests() async {
    emit(GetDonationsRequestsLoading());
    final result = await _orphanageDonationRemoteDataSource
        .getAdoptionRequests();
    result.fold(
  ...
  void getDonationItems() async {
    ...
    result.fold(
      ...
      (items) => emit(GetDonationsSuccess(items: items)),
    );
  }

  void respondToRequest(String requestId, String response) async {
    emit(RespondToRequestLoading());
    final result = await _orphanageDonationRemoteDataSource.respondToRequest(
      requestId,
      response,
    );
    result.fold(
